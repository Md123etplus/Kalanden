...
import { databases, DATABASE_ID, INSTRUCTOR_RATINGS_COLLECTION_ID, USERS_COLLECTION_ID } from '@/lib/appwrite'
...
const PopularInstructorsSection = () => {
  ...
  // Fetch popular instructors and their likes
  useEffect(() => {
    const fetchInstructors = async () => {
      try {
        const instructorsResponse = await databases.listDocuments(DATABASE_ID, USERS_COLLECTION_ID);
        const instructorsWithLikes = await Promise.all(
          instructorsResponse.documents.map(async (instructor: any) => {
            ...
            return {
              ...
              likes,
              coursesCreated: instructor.coursesCreated,
            };
          })
        );

        // Sort by likes in descending order and slice the top 3
        const topInstructors = instructorsWithLikes
