  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const response = await databases.listDocuments(DATABASE_ID, COURSES_COLLECTION_ID, queries)

        const coursesWithImages = await Promise.all(
          response.documents.map(async (course) => {
            let imageUrl = null
    ...
    fetchCourses()
  }, [searchQuery, selectedLevel, selectedSubject, quizzesOnly, showFreeOnly, searchParams]) // Added searchParams to dependencies

  const filteredCourses = courses.filter((course) => course.duration !== undefined && course.duration <= maxDuration)
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <div className="flex flex-col">
            <Label htmlFor="duration-filter" className="mb-2">
          ...
          <div className="flex items-center space-x-2">
            <Switch id="quizzes-filter" checked={quizzesOnly} onCheckedChange={setQuizzesOnly} />
            <Label htmlFor="quizzes-filter">Cours avec quiz uniquement</Label>
          </div>
          <div className="flex items-center space-x-2">
            <Switch id="free-filter" checked={showFreeOnly} onCheckedChange={setShowFreeOnly} />
